...
- **SWAP**: - **SWAP**: The item stays in function but gets replaced with a budget-friendly alternative that better fits the room's intended 
  palette. If the existing item is a clashing outlier (e.g., a navy pouf in a warm cream room), SWAP it toward the dominant mood, not  toward the outlier. Always include a color suggestion and approximate price.
